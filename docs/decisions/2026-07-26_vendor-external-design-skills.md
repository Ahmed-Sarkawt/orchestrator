...
1. `taste-skill` — verbatim from [Leonxlnx/taste-skill](https://github.com/Leonxlnx/taste-skill) `skills/taste-skill/SKILL.md` (MIT), with an attribution + repo-precedence header.
...
- Precedence headers resolve conflicts with CLAUDE.md hard rules: rule 7 (shadcn/ui base) and rule 4 (cross-browser animations) win for product UI; `taste-skill` applies to landing/marketing/portfolio surfaces only.
...
- `taste-skill` is large (~1,200 lines) but loads only when invoked — no per-session context cost.
